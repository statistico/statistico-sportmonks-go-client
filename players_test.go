...
func assertPlayerSeasonStats(t *testing.T, stats *PlayerSeasonStats) {
	...
	assert.Equal(t, 2, stats.Appearances)
	assert.Equal(t, 1, stats.Lineups)
	assert.Equal(t, 1, stats.SubstituteIn)
	assert.Equal(t, 0, stats.SubstituteOut)
	assert.Equal(t, 7, stats.SubstitutesOnBench)
	assert.Equal(t, 0, stats.Goals)
